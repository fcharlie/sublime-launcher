...
bool SublimeStartupProfile(const wchar_t *cf, SublimeStartupStructure &config) {
  ...
  if (cf == nullptr) {
    if (PathFileExistsW(L"launcher.toml")) {
      ...
      if (!GetProcessImageFileFolder(folder)) {
        MessageBoxW(nullptr, L"GetModuleFileNameW Failed!",
                    L"Internal System Error", MB_OK | MB_ICONERROR);
        return false;
      }
  ...
  try {
    ...
    MessageBoxA(nullptr, e.what(), "Parse toml file failed !",
                MB_OK | MB_ICONERROR);
    return false;
  }
...
bool ClearEnvironmentVariableW() {
  ...
  std::wstring path = L"${WINDIR}\\System32;${WINDIR};${WINDIR}"
                      L"\\System32\\Wbem;${WINDIR}"
...
bool StartupSublimeEnv(SublimeStartupStructure &config) {
  ...
  wchar_t cmdBuffer[ArraySize] = {0};
  if (!RebuildStartupCommandLine(config.executeFile, cmdBuffer, 32767)) {

    return false;
  }

  BOOL result = CreateProcessW(NULL, cmdBuffer, NULL, NULL, TRUE,
...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
  ...
  return StartupSublimeEnv(config) ? 0 : 2;
}
